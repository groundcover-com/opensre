...
``opensre --help`` reference (via :mod:`cli.interactive_shell.chat.grounding.cli_reference`)
and, for procedural questions, excerpts from ``docs/`` (via
:mod:`cli.interactive_shell.chat.grounding.docs_reference`). Neither surface includes
...
:mod:`cli.interactive_shell.chat.grounding.docs_reference` so the two stay symmetric.
...
import cli.interactive_shell.chat.grounding.grounding_diagnostics as _gd

# Repo root is four levels above this file
# (.../cli/interactive_shell/chat/grounding/agents_md_reference.py -> repo root).
