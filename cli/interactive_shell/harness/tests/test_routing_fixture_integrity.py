...
from cli.interactive_shell.harness.tests.scenario_loader import (
...
def test_executes_terminal_action_invariants() -> None:
    ...
    for case in load_all_scenarios():
        ...
        from cli.interactive_shell.harness.tests.scenario_loader import VALID_ACTION_KINDS
...
def test_routing_tests_are_fully_colocated() -> None:
    ...
    assert not unexpected, (
        "Routing tests must be colocated under cli/interactive_shell/harness/tests/. "
        "No routing tests should remain under tests/cli/interactive_shell/harness/: "
        + ", ".join(unexpected)
    )
