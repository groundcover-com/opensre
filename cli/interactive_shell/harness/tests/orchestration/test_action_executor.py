...
def test_run_cd_command_chdirs_to_target(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "cli.interactive_shell.harness.orchestration.action_executor.os.chdir",
        _chdir,
    )
...
def test_run_claude_code_implementation_starts_tracked_task(
    ...
    class _FakeProcess:
        ...
        def communicate(
            ...
            return "changed cli/interactive_shell\n", ""
...
def test_run_shell_command_silent_success_prints_checkmark(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "cli.interactive_shell.harness.orchestration.action_executor.execute_shell_command",
        _fake_execute,
    )
...
def test_run_opensre_agents_scan_prints_clean_foreground_output(
    ...
    monkeypatch.setattr(
        "cli.interactive_shell.harness.orchestration.action_executor.subprocess.run",
        _fake_run,
    )
...
def test_start_background_cli_task_uses_pty_for_live_terminal_output(
    ...
    monkeypatch.setattr(
        "cli.interactive_shell.harness.orchestration.action_executor.os.openpty",
        lambda: (10, 11),
        raising=False,
    )
    monkeypatch.setattr(
        "cli.interactive_shell.harness.orchestration.action_executor.os.read",
        _fake_read,
    )
    monkeypatch.setattr(
        "cli.interactive_shell.harness.orchestration.action_executor.os.close",
...
def test_start_background_cli_task_logs_failure_outcome_to_posthog(
    ...
    monkeypatch.setattr(
        "cli.interactive_shell.utils.telemetry.recorder.capture_ai_generation",
...
def test_start_background_cli_task_reports_watcher_failure(
    ...
    monkeypatch.setattr(
        "cli.interactive_shell.harness.orchestration.action_executor.read_diag",
...
def test_run_opensre_cli_command_skips_confirmation_for_investigate(
    ...
    monkeypatch.setattr(
        "cli.interactive_shell.harness.orchestration.action_executor.start_background_cli_task",
