...
def test_run_cd_command_chdirs_to_target(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "interactive_shell.harness.orchestration.action_executor.os.chdir",
        _chdir,
    )
...
def test_run_claude_code_implementation_starts_tracked_task(
    ...
    monkeypatch.setattr(
        "interactive_shell.harness.orchestration.action_executor.ClaudeCodeAdapter",
        _FakeAdapter,
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "interactive_shell.harness.orchestration.action_executor.threading.Thread",
        _ImmediateThread,
    )
...
def test_run_opensre_agents_scan_prints_clean_foreground_output(
    ...
    monkeypatch.setattr(
        "interactive_shell.harness.orchestration.action_executor.subprocess.run",
        _fake_run,
    )
...
def test_start_background_cli_task_uses_pty_for_live_terminal_output(
    ...
    monkeypatch.setattr(
        "interactive_shell.harness.orchestration.action_executor.os.openpty",
        lambda: (10, 11),
        raising=False,
    )
    monkeypatch.setattr(
        "interactive_shell.harness.orchestration.action_executor.os.read",
        _fake_read,
    )
    monkeypatch.setattr(
        "interactive_shell.harness.orchestration.action_executor.os.close",
...
def test_start_background_cli_task_reports_watcher_failure(
    ...
    monkeypatch.setattr(
        "interactive_shell.harness.orchestration.action_executor.read_diag",
...
def test_run_opensre_cli_command_skips_confirmation_for_investigate(
    ...
    monkeypatch.setattr(
        "interactive_shell.harness.orchestration.action_executor.start_background_cli_task",
