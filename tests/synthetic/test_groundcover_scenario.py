...
def test_groundcover_alert_source_maps_to_tools() -> None:
    """A groundcover alert source seeds groundcover tools before the ReAct loop."""
    assert "groundcover" in ALERT_SOURCE_TO_SEED_TOOL_SOURCES
    assert ALERT_SOURCE_TO_SEED_TOOL_SOURCES["groundcover"] == ("groundcover",)


def test_logs_scenario() -> None:
    backend = _FixtureGroundcoverBackend()
    result = query_groundcover_logs(
        query="level:error workload:checkout | fields _time, content | limit 50",
...
def test_traces_scenario() -> None:
    backend = _FixtureGroundcoverBackend()
    result = query_groundcover_traces(
        query="status:error | fields _time, span_name, duration_seconds | limit 50",
        groundcover_backend=backend,
    )
    assert result["available"] is True
...
def test_apm_scenario() -> None:
    ...
    result = query_groundcover_apm(
        query="resource_type:http is_inbound:true source:ebpf "
...
def test_events_entities_monitors_issues_scenario() -> None:
    ...
    events = query_groundcover_events(
        query="type:Warning | fields _time, reason, namespace, message | limit 50",
        groundcover_backend=backend,
    )
    entities = query_groundcover_entities(
        query="kind:Pod | fields name, namespace, status_phase | limit 50",
        groundcover_backend=backend,
    )
    monitors = query_groundcover_monitors(query="", groundcover_backend=backend)
    issues = query_groundcover_issues(
        query="* | fields monitor_name, last_firing_start | limit 50",
        groundcover_backend=backend,
    )
