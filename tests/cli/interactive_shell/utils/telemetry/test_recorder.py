...
def test_prompt_recorder_start_respects_supported_routes(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "cli.interactive_shell.utils.telemetry.recorder.PromptLogConfig.load", lambda: cfg
    )
    session = ReplSession()
...
def test_prompt_recorder_sends_ai_generation(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "cli.interactive_shell.utils.telemetry.recorder.build_turn_integration_snapshot",
        lambda _session: {
...
def test_prompt_recorder_still_captures_when_tool_resolution_fails(
    ...
    monkeypatch.setattr(
        "cli.interactive_shell.utils.telemetry.integration_snapshot.get_available_tools",
        _boom,
    )
