...
Start the interactive shell with `opensre` (TTY required). Type a slash command at the prompt, or describe what you want in plain language — the action planner can select the right command.
...
| --- | --- |
| Slash command (`/status`) | Planned as an explicit command action and executed by the REPL |
| Plain language (`verify datadog`) | Mapped by the action planner to slash commands, investigations, or doc-grounded answers |
